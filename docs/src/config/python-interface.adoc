...
== Introduction
User interfaces control LinuxCNC activity by sending commands to the LinuxCNC task controller,
and monitor results by observing the LinuxCNC status structure, as well as the error reporting channel.

They reach task over a websocket carrying FlatBuffers.
Programmatic access is through a C++ API, `src/emc/ws_client`;
the most important parts of it
are also available to Python programs through the `linuxcnc` module.

Beyond the command, status and error interfaces, the `linuxcnc` module also contains:
...
== Usage Patterns for the LinuxCNC interface

The general pattern for `linuxcnc` usage is roughly like this:

- Import the `linuxcnc` module.
- Establish connections to the command, status and error interfaces as needed.
...
The linuxcnc module connects to task on `127.0.0.1` at the port given by
`[TASK]WEBSOCKET_PORT` in `$INI_FILE_NAME`, default 5700. `LINUXCNC_WS_PORT`
overrides both.
...
Operator messages are broadcast: every connected client is sent its own copy,
so reading one here does not take it away from AXIS or any other UI.
This was not true of the NML error channel, which was a single queue whose
first reader consumed the message -- with two UIs attached, which one saw a
given message depended on timing, and only one error reader per setup was
recommended.
...
bool = ini.hasvariable(_section_:string, _variable_:string [, _num_:int])::
  ...
  The optional _num_ argument may be used to test whether the num'th _variable_ exists in the _section_.
...
tuple(filename,lineno) = ini.lineof(_section_:string, _variable_:string [, _num_:int])::
  ...
  The first matching _section_ _variable_ is returned if _num_ is not provided.
...
fname = sys.argv[1] if len(sys.argv) > 1 else "<missing filename>"
try:
    inifile = linuxcnc.ini(fname)
except linuxcnc.error as detail:
    print("{}: error: {}".format(fname, detail))
    sys.exit(1)
