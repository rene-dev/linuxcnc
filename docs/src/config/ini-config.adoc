...
* `TASK = milltask` -
  ...
  - communicate with the UIs over the websocket transport,
...
* `WEBSOCKET_PORT = 5700` -
  The port task listens on for user interfaces.
  Status, commands and operator messages all travel over this one connection.
  The listener is bound to the loopback address, so only programs on the same machine can reach it.
  There is normally no reason to set this: a user interface uses the same default, and only a machine running two instances of LinuxCNC at once needs to tell them apart.
  Setting it to 0 switches the transport off entirely, which stops `linuxcncrsh`, `linuxcnclcd`, `schedrmt` and tklinuxcnc from connecting at all.
