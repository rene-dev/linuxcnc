...
motion, axis - accepts motion commands, interacts with HAL in realtime
...
*motion.adaptive-feed* IN FLOAT::
  ...
  task-level feed override value and motion.feed-hold. Negative values
...
*motion.switchkins-type* IN float::
  ...
  the pin value.
  *Deprecated*: the interpreter does not see this pin, so limits and
  look ahead go on using the kinematics it last knew about. Use the
  G-code words *G12.1 P-* and *G13.1*, which ask motion directly and
  synchronize task and motion themselves. Motion reports the
