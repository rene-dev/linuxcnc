...
count-f (float out)::
 The same count as a float, taken from the full-width internal counter so it
 does not wrap at 2^31. Use this for *axis.__L__.jog-counts* and
 *joint.__N__.jog-counts*, which are float pins.
position (float out)::
