...
halui - HAL pins to observe and command LinuxCNC

== DESCRIPTION

*halui* is used to build a User Interface using hardware knobs and switches.
It provides a large number of HAL pins, and acts accordingly when these
change.
...
the pins you want in a HAL file. Earlier versions ran halui as a separate
program, selected with *[HAL]HALUI* in the INI file; that setting is no
longer read.

== USAGE

A user can connect the pins to his physical knobs & switches & leds, and
when a change is noticed halui triggers an appropriate event.
