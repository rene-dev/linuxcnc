...
- In addition to being connected to 'touchy.wheel-counts', the wheel counts
  ...
  each axis _N_.  Both are float pins, so a jog wheel widget's float output
  (for instance a gladevcp dial's '-f' pin) is what you want here.
  If you use HAL component 'lowpass' to smooth wheel jogging, be
