...
# jog-counts is a float pin, so use the float twin of the counts pin.
net encoder-counts  <= encoder.0.counts-f
...
for each click of the MPG use the link:../man/man9/lowpass.9.html[lowpass] component to
limit the acceleration.

.jog.hal with lowpass
[source,hal]
----
loadrt encoder num_chan=1
loadrt mux4 count=1
...
# The output from encoder counts is sent to lowpass
net mpg-out lowpass.0.in <= encoder.0.counts-f
...
# The output from the lowpass is sent to each axis jog count
# Only the selected axis will move.
net encoder-counts  <= lowpass.0.out
net encoder-counts => axis.x.jog-counts
net encoder-counts => axis.y.jog-counts
net encoder-counts => axis.z.jog-counts
----
