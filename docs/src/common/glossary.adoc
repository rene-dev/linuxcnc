...
NML::
     (((NML))) Neutral Message Language. The message transport LinuxCNC
     used between the UIs and task until the 2.10 development cycle,
     carried over shared memory by libnml. Replaced by FlatBuffers over a
     websocket; the term survives only in some class names.
Offsets::
     (((offsets)))
