...
[[gcode:canned-cycle-direction]]
=== Cycle Direction

The hole goes from the R position towards the Z position. Usually R is
above Z, and the cycle works in the negative Z direction. If R is below
Z, the cycle works in the positive Z direction, and all retract moves go
in the negative Z direction. This can be used for example for
horizontal drilling in the 'G18' or 'G19' plane, when the spindle
points in the positive direction of the axis perpendicular to the plane.

In the descriptions of the canned cycles, 'up', 'above', 'higher' and
'out' mean away from the hole towards the R position, and 'down',
'below', 'lower' and 'deep' mean towards the Z position.

CAUTION: If R and Z are swapped by mistake, the cycle does not stop with
an error, it rapids to R and cuts in the opposite direction.
...
if the Z number is missing and the same canned cycle was not already
active.
