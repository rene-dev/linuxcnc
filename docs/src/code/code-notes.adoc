...
the task controller into motion commands and sent to the motion controller.
...
* 'motmod' - processes motion commands and controls hardware via HAL
...
enable input is false") if the motion.enable HAL pin is low. That pin is
created with a default value of TRUE, so a machine that leaves it
unconnected is never refused.
...
planner brings the feed to zero within the current segment, at that
segment's acceleration limit, and halts wherever the ramp ends.

Pause is ignored while the active segment is position-synchronized with
the spindle (G33 threading, rigid tapping): there the planner forces full
feed and bypasses both pause and feed override, and the pause takes effect only when the segment ends. Velocity-synchronized segments (G95 feed per revolution) can be
paused normally.
...
task over its own message channels; the HAL component is still named
...
== Messages between the UIs and task

A UI reaches task over a websocket carrying FlatBuffers. The schema is in
`src/emc/flatbuf/*.fbs` and the encoders and decoders are generated from
it; `src/emc/task/ws_server.cc` is the task side and
`src/emc/ws_client/ws_client.{cc,hh}` the client side. `[TASK]WEBSOCKET_PORT`
sets the port, default 5700, loopback only.

One connection carries what used to be three channels: status snapshots
published once per task cycle, commands in, and per-session acknowledgements
and operator messages out. Because an acknowledgement is addressed to the
session that sent the command, two UIs attached at once no longer read each
other's results.

In the code a command is still an `EMC_*` object deriving from
`RCS_CMD_MSG`, and status is still `EMC_STAT`; those live in
`src/emc/nml_intf/emc_nml.hh` on top of `src/emc/nml_msg.hh`. They are
plain structures now, with no transport in them -- the names are the ones
NML left behind.

=== Adding a message type

. add the class to `src/emc/nml_intf/emc_nml.hh`, deriving from the right
  `EMC_*_CMD_MSG` base
. give it a type number there as well, `#define EMC_IO_GENERIC_TYPE \((NMLTYPE) 1605)`
. add its constructor to `src/emc/nml_intf/emcops.cc`
. add a case to `emc_symbol_lookup()` in `src/emc/nml_intf/emc.cc`, so it
  has a name in debug traces and on the interpreter list
. add it to `src/emc/flatbuf/emc_cmd.fbs`, and a case to `decode_command()`
  in `ws_server.cc` and to `encode_command()` in `ws_client.cc` -- those two
  are mirrors of each other

A message that only travels inside task, such as one queued onto the
interpreter list, needs the first four steps and not the fifth.

=== Historical note

Until the 2.10 development cycle this was NML (Neutral Message Language),
carried by libnml -- a cut-down NIST rcslib -- over shared memory, with
`linuxcncsvr` offering the same buffers over TCP for remote UIs. `src/libnml`,
the `.nml` configuration files and `[EMC]NML_FILE` are all gone. Anything
that opened those channels directly has to move to the websocket; there is
