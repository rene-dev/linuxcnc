...
typedef struct __connectionrectype_t connectionRecType;   // Forward decl.

typedef struct __process_state_t {
    process_enum_t state;   // Waiting sequence for loadusr -W/-w
    volatile pid_t pid;     // Also set by the signal handler
    volatile int retval;    // Set by the signal handler (< 0 if signal terminated process)
    std::string acknak;     // The command in the answer after it finishes
    int (*cascade)(connectionRecType &);    // Async wait function to cascade into (NULL if done)
    std::string insmod;     // For loadrt's insmod string set in HAL
    std::string name_proc;  // Process name (executable)
    std::string name_comp;  // Component name (explicitly set with loadusr -n compname)
    std::set<std::string> comp_names_pre;
    bool wait_comp;         // Set on loadusr -W
    bool wait_proc;         // Set on loadusr -w
    bool ignore_rv;         // Set on loadusr -i
    bool ready;             // The component was detected ready
    volatile bool exited;   // Process exited, also set by the signal handler
    bool pacify;            // Set when the "Waiting for..." text needs termination
    double timestamp;       // Start of the new process
    int nextmsg;            // Progressive second count for pacifier message
    ...
    void clear() {
        ...
        exited    = false;
        pacify    = false;
        timestamp = mytimeofday();
        nextmsg   = 3;
    };
} process_state_t;

typedef struct __connectionrectype_t {
    ...
    process_state_t process;        // Async state for doLoadUsr
    bool halfclosed;
    bool linked;
    bool enabled;
...
// A child died and we need to wait for it to remove the zombie.
static void sigChld(int sig)
{
    (void)sig;
    while(true) {   // Need to catch all zombies
        int status = 0;
        int rv = waitpid(0, &status, WNOHANG);
        if(rv <= 0)
            return;  // On error or no zombies left

        if(WIFSTOPPED(status) || WIFCONTINUED(status))
            continue;

        // Get here and some process terminated. Try to find the
        // associated process and see if we were waiting for it.
        for(auto &cl : clients) {
            if(rv == cl.process.pid) {
                cl.process.pid = 0;
                cl.process.exited = true;
                // The process terminated normally or on a signal
                if(WIFEXITED(status)) {
                    cl.process.retval = WEXITSTATUS(status);
                } else {
                    cl.process.retval = -WTERMSIG(status);
                }
                break;
            }
        }
...
// FIXME: This is not done using a coroutine. It blocks the main loop and all
// clients.
static int halrmt_systemv(connectionRecType &ctx, const std::vector<const char *> &argv)
{
    ...
    // Block SIGCHILD until we have registered the PID or we might miss to
    // capture the exit. The signal handler searches for the PID.
    sigset_t sigs;
    sigemptyset(&sigs);
    ...
    ctx.process.pid = halrmt_systemv_nowait(ctx, argv);
    int err = errno;

    // Restore old sigmask now we have the registration in place
    ...
    while(true) {
        if(ctx.process.exited) {
            break;
        }
        mysleep(0.01);
    }

    if(ctx.process.retval > 0) {
        errornl(ctx, fmt::format("{}: exit value: {}", argv[0], ctx.process.retval));
        return ctx.process.retval;
    } else if(ctx.process.retval < 0) {
        errornl(ctx, fmt::format("{}: exit on signal {}", argv[0], -ctx.process.retval));
        return -EINTR;
    }
    return 0;
}
...
{
    if(args.size() < 3) {
        // We need: dummy prog <null>
        // The dummy is for getopt.
        errornl(ctx, fmt::format("internal: doLoadUsr: Not enough arguments"));
        return -EINVAL;
    }
    if(NULL != args[args.size() - 1]) {
        errornl(ctx, fmt::format("internal: doLoadUsr: Argument list not NULL terminated"));
        return -EINVAL;
    }

    if(hal_get_lock() & HAL_LOCK_LOAD) {
        errornl(ctx, "doLoadUsr: HAL is locked, loading of programs is not permitted");
        return -EPERM;
    }

    ctx.process.clear();

    // This is a loadusr call
    // Check for options (-w, -i, -W and/or -n)
    reset_getopt_state();
    int optc;
    int argc = args.size() - 1; // Skip the terminating NULL in the count
    while (-1 != (optc = getopt(argc, (char * const *)args.data(), "+wWin:"))) {
        switch(optc) {
        case 'w': ctx.process.wait_proc = true; break;
        case 'W': ctx.process.wait_comp = true; break;
        case 'i': ctx.process.ignore_rv = true; break;
        case 'n': ctx.process.name_comp = optarg; break;
        default:
            errornl(ctx, fmt::format("doLoadUsr: Invalid option '-{}'", isprint(optc) ? optc : '?'));
            return -EINVAL;
        }
    }
    // Collect the remainder as the program to execute
    std::vector<const char *> argv;
    ...
    if(argv.size() < 2) {
        errornl(ctx, fmt::format("doLoadUsr: Not enough arguments for halrmt_systemv. Missing program name?"));
        return -EINVAL;
    }
    if(NULL != argv[argv.size() - 1]) {
        errornl(ctx, fmt::format("internal: doLoadUsr: Argument list not NULL terminated for halrmt_systemv"));
        return -EINVAL;
    }

    ctx.process.name_proc = argv[0];
    // Get component name
    if(ctx.process.name_comp.empty()) {
        ctx.process.name_comp = guess_comp_name(argv[0]);
    }

    // Component status quo
    ctx.process.comp_names_pre = get_all_comp_names();
    ...
    if(ctx.process.pid < 0) {
        errornl(ctx, fmt::format("doLoadUsr: Failed to execute '{}'", argv[0]));
        ctx.process.pid = 0;
        ctx.process.exited = true;
        return -err;
    }

    // The wait sequence can be one of:
    //   - wait component, wait process
    //   - wait component
    //   - wait process
    if(ctx.process.wait_comp) {
        ctx.process.state = PS_WAITCOMP;
    } else if(ctx.process.wait_proc) {
        ctx.process.state = PS_WAITPROC;
    }
    return 0;
}

static void waitPacify(connectionRecType &ctx)
{
    // Pacify the user on long waits
    double delta = mytimeofday() - ctx.process.timestamp;
    if(delta >= ctx.process.nextmsg) {
        if(0 == (ctx.process.nextmsg - 3) % 30) {
            reply(ctx, fmt::format("{}Waiting for component '{}' to become ready.",
                        ctx.process.pacify ? "\n" : "", ctx.process.name_comp));
            warn_newly_loaded_comps(ctx, ctx.process.comp_names_pre, ctx.process.name_comp);
        } else {
            reply(ctx, ".");
            warn_newly_loaded_comps(ctx, ctx.process.comp_names_pre, ctx.process.name_comp);
        }
        ctx.process.nextmsg++;
        ctx.process.pacify = true;
    }
}

static int waitComponent(connectionRecType &ctx)
{
    ...
    ctx.process.ready = !rv && q.comp.ready;

    if(ctx.process.ready) {
        return 0;   // Done
    }

    // This happens when SIGCHLD has hit
    if(ctx.process.exited) {
        // Getting here means the component does not exist and the process
        // responsible for creating it is dead. For rtapi_app, it called
        // rtapi_app_main() and waits for it to be done. This is where the
        // component should have been created.
        if(ctx.process.pacify) {
            replynl(ctx, "");
            ctx.process.pacify = false;
        }
        if(-ENOENT != rv) {
            errornl(ctx, fmt::format("searching for component '{}' failed with error={} ({})",
                        ctx.process.name_comp, rv, hal_strerror(rv)));
        }
        if(ctx.process.retval < 0) {
            errornl(ctx, fmt::format("program '{}' terminated on signal {} with no component",
                        ctx.process.name_proc, -ctx.process.retval));
        } else {
            errornl(ctx, fmt::format("program '{}' terminated with code {} with no component",
                        ctx.process.name_proc, ctx.process.retval));
        }
        ctx.process.state = PS_IDLE;
        return 0;
    }

    // Process still alive and component not found or not ready yet.
    waitPacify(ctx);
    return 1;   // Not done yet

}

static int processDoLoadUsr(connectionRecType &ctx)
{
    int rv;

    switch(ctx.process.state) {
    default:
        errornl(ctx, fmt::format("internal: processDoLoadUsr: Invalid state {}", (int)ctx.process.state));
        ctx.process.state = PS_IDLE;
        return 0;
    ...
    case PS_WAITCOMP:
        ...
        if(rv <= 0) {
            ...
            ctx.process.state = ctx.process.wait_proc ? PS_WAITPROC : PS_IDLE;
        } else {
            flushall();
        }
        return rv > 0;
    ...
    case PS_WAITPROC:
        ...
        if(ctx.process.exited) {
            // The SIGCHLD handler caught the exit
            if(ctx.process.retval < 0) {
                // The process died on a signal
                errornl(ctx, fmt::format("doLoadUsr: program '{}' terminated on signal {}",
                        ctx.process.name_proc, -ctx.process.retval));
            } else if(!ctx.process.ignore_rv) {
                if (0 != ctx.process.retval) {
                    errornl(ctx, fmt::format("doLoadUsr: program '{}' failed, returned {}",
                            ctx.process.name_proc, ctx.process.retval));
                }
            }
            // Done waiting
        ...
        return 1;   // Process still going
    }
}

static int processDoLoadRtInsmod(connectionRecType &ctx)
{
    // Module argument string needs to go into hal memory
    char *cptr = (char *)hal_malloc(ctx.process.insmod.size() + 1);
    if (!cptr) {
        errornl(ctx, "LoadRt: Failed to allocate HAL memory");
        ctx.process.retval = -ENOMEM;
    } else {
        strcpy(cptr, ctx.process.insmod.c_str());
        int rv = hal_comp_insmod_args(ctx.process.name_comp.c_str(), cptr);
        if(rv) {
            errornl(ctx, fmt::format("LoadRt: insmod args for module '{}' failed, error={} ({})",
                        ctx.process.name_comp, rv, hal_strerror(rv)));
            ctx.process.retval = -EINVAL;
        } else {
            ctx.process.retval = 0;
        }
    }
    return 0;
}
...
#if defined(RTAPI_USPACE)
static int processDoLoadRtWait(connectionRecType &ctx)
{
    // First wait for the process to finish. Then cascade into the insmod
    // assignment for HAL.
    if(!processDoLoadUsr(ctx)) {
        ctx.process.cascade = &processDoLoadRtInsmod;
    }
    return 1;
}
#endif
...
static int doLoadRt(connectionRecType &ctx, const std::vector<std::string> &args)
{
    ...
    std::string mod_name = args[0];

#if defined(RTAPI_USPACE)
    argv.push_back("loadrt");   // For getopt(), will be ignored
    argv.push_back("-W");
    argv.push_back("-n");
    argv.push_back(mod_name.c_str());
    argv.push_back(EMC2_BIN_DIR "/rtapi_app");
    ...
    if (0 != retval) {
        errornl(ctx, fmt::format("LoadRt: rtapi_app load '{}' failed, returned {}", mod_name, retval));
        return retval;
    }
    ...
    std::string mod_path = fmt::format("{}/{}{}", EMC2_RTLIB_DIR, mod_name, MODULE_EXT);

    // Does the module exist?
    struct stat stat_buf;
    if (stat(mod_path.c_str(), &stat_buf) != 0 ) {
        errornl(ctx, fmt::format("LoadRt: Can't find module '{}' in '{}'", mod_name, EMC2_RTLIB_DIR));
        return -ENOENT;
    }
    ...
    if (0 != retval) {
        errornl(ctx, fmt::format("LoadRt: insmod '{}' failed, returned {}", mod_name, retval));
        return retval;
    }

    ctx.process.name_comp = mod_name;
...
{
    // GET TIME
    replynl(ctx, fmt::format("TIME {}", mytimeofday()));
    return rtOk;
}
...
static cmdResponseType setLoadUsr(connectionRecType &ctx)
{
    ...
    std::vector<const char *> argv;
    argv.push_back("loadusr"); // Will be ignored
    for(unsigned i = 2; i < ctx.toks.size(); i++) {
    ...
    if(0 == rv) {
        ctx.process.cascade = &processDoLoadUsr;
        ctx.process.acknak = "LOADUSR";
        return rtDelayed;
    }
    return rtError;
}
...
static cmdResponseType setSave(connectionRecType &ctx)
{
    ...
    if(ctx.toks.size() > 3) {
        ...
        if(fd < 0) {
            errornl(ctx, fmt::format("Cannot open '{}' for write ({})", ctx.toks[3], strerror(errno)));
            return rtError;
        }
        size_t done = 0;
        while(done < out.size()) {
...
static std::string resolveIni(connectionRecType &ctx, const std::string &section, const std::string &variable)
{
    ...
    if(auto str = ini.findString(variable, section))
        return *str;
    errornl(ctx, fmt::format("Ini variable '[{}]{}' not found", section, variable));
    return "";
}

static std::string resolveEnv(connectionRecType &ctx, const std::string &variable)
{
    const char *cptr = getenv(variable.c_str());
    if(!cptr) {
        errornl(ctx, fmt::format("Environment variable '{}' not found", variable));
        return "";
    }
    return cptr;
}
...
//
// Expand embedded INI-file and envronment references in form of:
// - [section]variable
// - [section](variable)
// - $envvar
// - $(envvar)
//
static void lineExpand(connectionRecType &ctx, std::string &line)
{
    if(!ctx.expand)
        return;
    std::string newline;
    std::string section;
    std::string variable;
    enum { ST_START, ST_BOPEN, ST_BCLOSE, ST_POPEN, ST_VAR, ST_ENV, ST_ENVPO, ST_ENVVAR } state = ST_START;
    for(const auto &ch : line) {
        ...
        case ST_ENV: // Seen '$'
            ...
            break;
        case ST_ENVVAR: // Seen '$x'
            if(!isidchar(ch)) {
                if(isspace(ch & 0xff)) {
                    // Have '$variable' --> resolve
                    newline += resolveEnv(ctx, variable);
                } else {
                    // Invalid termination --> restore
                    newline += '$' + variable;
                }
    ...
    case ST_ENV:    newline += '$'; break;
    case ST_ENVVAR: newline += '$' + variable; break;
    case ST_ENVPO:  newline += "$(" + variable; break;
    default: errornl(ctx, fmt::format("internal: Invalid state {} in lineExpand()", (int)state)); break;
    }
    line = newline;
}
...
static int sockMain(int svrfd)
{
    ...
    while (!quitloop && (clients.size() || !shutdown)) {
        ...
        for (size_t i = 0; i < clients.size(); i++) {
            ...
            // If there is a process launch in progress, don't poll wait
            // forever, but use 10ms timeout.
            if (clients[i].process.cascade && 0 != pto) {
                pto = 10;
            } else if (clients[i].inbuf.find_first_of("\r\n") != std::string::npos) {
        ...
        for (auto i = clients.begin(); i != clients.end(); ++i) {
            // Handle the process wait situation.
            if (i->process.cascade) {
                // Zero return means we're done
                if(i->process.cascade(*i)) {
                    continue; // Non-zero, keep going
                }
                // Async processing finished
                i->process.cascade = NULL;
                if(!i->process.acknak.empty()) {
                    if(i->verbose || i->process.retval)
                        replynl(*i, fmt::format("SET {} {}", i->process.acknak, i->process.retval ? "NAK" : "ACK"));
                    i->process.acknak.clear();
                }
            }
...
int main(int argc, char **argv)
{
    ...
    // Catch termination of our sub-processes
