...
static void hm2_read_request(void *void_hm2, long period) {
    ...
    if (*hm2->llio->hal->io_error != 0) return;
...
int hm2_register(hm2_lowlevel_io_t *llio, char *config_string) {
...

    {
        ...
        llio->hal = hal_malloc(sizeof(*llio->hal));
        if (llio->hal == NULL) {
            HM2_ERR("out of memory!\n");
            r = -ENOMEM;
            goto fail0;
        ...
        r = hal_pin_bit_new(name, HAL_OUT, &hm2->llio->hal->io_error, llio->comp_id);
        *hm2->llio->hal->io_error = 0;
        if (r < 0) {
